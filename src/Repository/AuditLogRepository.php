...
use BlackCat\Database\Packages\AuditLog\Dto\AuditLogDto as Dto;
use BlackCat\Database\Packages\AuditLog\Mapper\AuditLogDtoMapper as RowMapper;
...
final class AuditLogRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\AuditLog\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\AuditLog\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\AuditLog\Dto\AuditLogDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\AuditLog\Dto\AuditLogDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\AuditLog\Mapper\AuditLogDtoMapper::fromRow($row);
    }
