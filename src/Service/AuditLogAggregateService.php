 ...
 */
final class AuditLogAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private AuditLogRepository $auditLogRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'audit_log' => $this->auditLogRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\AuditLog\ModuleException("Record {$idStr} not found for locking.");
            }
