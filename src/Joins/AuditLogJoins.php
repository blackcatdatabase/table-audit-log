...
namespace BlackCat\Database\Packages\AuditLog\Joins;
...
final class AuditLogJoins {
    ...
    /**
     * FK: audit_log -> users
     * LEFT JOIN vw_users AS $as ON $as.id = $alias.changed_by
     * @return array{0:string,1:array}
     */
    public function joinUsers(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_users AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.changed_by' . ' ', []];
    }
