...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 1 outbound / 1 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 2 · Changelog: fresh (2 d)</div>
</div>
...
- Pkg folder: [packages\audit-log](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\audit-log)
...
graph LR
  ...
  audit_log["audit_log"]:::center
  audit_log -->|FK| users["users"]:::link
  audit_chain["audit_chain"]:::inbound -->|FK| audit_log
...
- Inbound (relies on this): "audit_chain"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package audit-log -Apply
